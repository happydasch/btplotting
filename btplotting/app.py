...
from .utils import get_dataname, get_datanames, get_plotobjs, filter_obj
...
class BacktraderPlotting(metaclass=bt.MetaParams):
    ...
    def __init__(self, **kwargs):
        if not isinstance(self.p.scheme, Scheme):
            raise Exception(
                "Provided scheme has to be a subclass"
        ...
        if self.p.use_default_tabs:
            self.tabs += [AnalyzerTab, MetadataTab, SourceTab, LogTab]
        if not isinstance(self.tabs, list):
            raise Exception("Param tabs needs to be a list containing tabs to display")
    ...
    def _blueprint_strategy(self, figid=0, filterdata=None):
        ...
        for parent, childs in objs.items():
            figure = Figure(fp=fp, scheme=scheme, master=parent, childs=childs)
            figure.plot(parent)
            for c in childs:
                figure.plot(c)
            figure.apply()
    ...
    def _blueprint_optreturn(self, figid=0):
        ...
        fp.reset()
        fp.analyzers += [a for _, a in optreturn.analyzers.getitems()]
    ...
    def _output_plotfile(self, model, figid=0, filename=None, template="basic.html.j2"):
        """
    ...
    def create_figurepage(
        self, obj, figid=0, start=None, end=None, filterdata=None, filldata=True
    ):
        """
    ...
    def generate_bokeh_model_tab_panels(self, figid=0):
        ...
        # sort figures
        data_sort = {False: 0}
        for i, d in enumerate(get_datanames(fp.strategy, onlyplotable=False), start=1):
            data_sort[d] = i
        ...
        for tab in tabs:
            ...
            if self.scheme.xaxis_pos == "bottom":
                for i, x in enumerate(tabs[tab]):
                    x.figure.xaxis.visible = False if i < len(tabs[tab]) - 1 else True
    ...
    def get_data(
        self, figid=0, startidx=None, start=None, end=None, back=None
    ) -> pd.DataFrame:
        """
    ...
    def plot(
        ...
        **kwargs,
        ...
        self.create_figurepage(
            obj, figid=figid, start=start, end=end, filterdata=filterdata
        )
        ...
        for figid in self._figurepages:
            ...
            if self.p.output_mode in ["show", "save"]:
                if self._iplot:
                    ...
                    filename = self._output_plotfile(model, figid, self.p.filename)
    ...
    def save_png(
        ...
        image = get_screenshot_as_png(
            model, driver=driver, timeout=timeout, resources=resources
        )
